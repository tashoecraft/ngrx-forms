...
describe(`form array ${markAsTouchedReducer.name}`, () => {
  ...
  it('should mark itself and all children recursively as touched', () => {
    const resultState = markAsTouchedReducer(INITIAL_STATE, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should not update state if all children are marked as touched recursively', () => {
    const resultState = markAsTouchedReducer(INITIAL_STATE_TOUCHED, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should mark children as touched if the group itself is already marked as touched', () => {
    ...
    const resultState = markAsTouchedReducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).toEqual(INITIAL_STATE_TOUCHED);
  });
  ...
  it('should mark group children as touched', () => {
    const resultState = markAsTouchedReducer(INITIAL_STATE_NESTED_GROUP, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isTouched).toEqual(true);
    expect(resultState.controls[0].isUntouched).toEqual(false);
  });

  it('should mark array children as touched', () => {
    const resultState = markAsTouchedReducer(INITIAL_STATE_NESTED_ARRAY, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isTouched).toEqual(true);
    expect(resultState.controls[0].isUntouched).toEqual(false);
  });

  it('should forward actions to children', () => {
    const resultState = markAsTouchedReducer(INITIAL_STATE, MarkAsTouchedAction({controlId: INITIAL_STATE.controls[0].id}));
