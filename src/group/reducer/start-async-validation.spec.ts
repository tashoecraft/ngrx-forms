...
describe(`form group ${startAsyncValidationReducer.name}`, () => {
  ...
  it('should mark state as having validation pending if control child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE, StartAsyncValidationAction({controlId: FORM_CONTROL_INNER_ID, name}));
  ...
  it('should mark state as having validation pending if group child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE_FULL, StartAsyncValidationAction({controlId: FORM_CONTROL_INNER3_ID, name}));
    expect(resultState.isValidationPending).toEqual(true);
  });

  it('should mark state as having validation pending if array child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE_FULL, StartAsyncValidationAction({controlId: FORM_CONTROL_INNER5_ID, name}));
    expect(resultState.isValidationPending).toEqual(true);
  });
});
