...
describe(`form array ${disableReducer.name}`, () => {
  it('should update state if enabled', () => {
    const resultState = disableReducer(INITIAL_STATE, DisableAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should not update state if disabled', () => {
    ...
    const resultState = disableReducer(state, DisableAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should disable group children', () => {
    const resultState = disableReducer(INITIAL_STATE_NESTED_GROUP, DisableAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isEnabled).toBe(false);
    expect(resultState.controls[0].isDisabled).toBe(true);
  });

  it('should disable array children', () => {
    const resultState = disableReducer(INITIAL_STATE_NESTED_ARRAY, DisableAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isEnabled).toBe(false);
    expect(resultState.controls[0].isDisabled).toBe(true);
  });

  it('should forward actions to children', () => {
    const resultState = disableReducer(INITIAL_STATE, DisableAction({controlId: INITIAL_STATE.controls[0].id}));
