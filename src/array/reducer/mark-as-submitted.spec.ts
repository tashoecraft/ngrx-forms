...
describe(`form array ${markAsSubmittedReducer.name}`, () => {
  ...
  it('should mark itself and all children recursively as submitted', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should not update state if all children are marked as submitted recursively', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE_SUBMITTED, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should mark children as submitted if the group itself is already marked as submitted', () => {
    ...
    const resultState = markAsSubmittedReducer(state, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).toEqual(INITIAL_STATE_SUBMITTED);
  });
  ...
  it('should mark group children as submitted', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE_NESTED_GROUP, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isSubmitted).toEqual(true);
    expect(resultState.controls[0].isUnsubmitted).toEqual(false);
  });

  it('should mark array children as submitted', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE_NESTED_ARRAY, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isSubmitted).toEqual(true);
    expect(resultState.controls[0].isUnsubmitted).toEqual(false);
  });

  it('should forward actions to children', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE, MarkAsSubmittedAction({controlId: INITIAL_STATE.controls[0].id}));
