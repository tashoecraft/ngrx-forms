...
describe(`form array ${startAsyncValidationReducer.name}`, () => {
  ...
  it('should update state with pending validation', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE, StartAsyncValidationAction({controlId: FORM_CONTROL_ID, name}));
  ...
  it('should update state with pending validation if validations are already pending', () => {
    ...
    const resultState = startAsyncValidationReducer(state, StartAsyncValidationAction({controlId: FORM_CONTROL_ID, name}));
  ...
  it('should mark state as having validation pending if control child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE, StartAsyncValidationAction({controlId: FORM_CONTROL_0_ID, name}));
  ...
  it('should mark state as having validation pending if group child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE_NESTED_ARRAY, StartAsyncValidationAction({controlId: FORM_CONTROL_0_ID, name}));
    expect(resultState.isValidationPending).toEqual(true);
  });

  it('should mark state as having validation pending if array child is marked as having validation pending', () => {
    const name = 'required';
    const resultState = startAsyncValidationReducer(INITIAL_STATE_NESTED_GROUP, StartAsyncValidationAction({controlId: FORM_CONTROL_0_ID, name}));
    expect(resultState.isValidationPending).toEqual(true);
  });
});
