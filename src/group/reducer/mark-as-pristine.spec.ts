...
describe(`form group ${markAsPristineReducer.name}`, () => {
  it('should update state if dirty', () => {
    ...
    const resultState = markAsPristineReducer(state, MarkAsPristineAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.isDirty).toEqual(false);
    expect(resultState.isPristine).toEqual(true);
  });

  it('should not update state if pristine', () => {
    const resultState = markAsPristineReducer(INITIAL_STATE, MarkAsPristineAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should forward actions to children', () => {
    ...
    const resultState = markAsPristineReducer(state, MarkAsPristineAction({controlId: state.controls.inner.id}));
