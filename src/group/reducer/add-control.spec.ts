...
describe(`form group ${addControlReducer.name}`, () => {
  ...
  it('should create child state for group child', () => {
    ...
    const action = AddGroupControlAction({controlId: FORM_CONTROL_ID, name: 'inner3', value});
    const resultState = addControlReducer(INITIAL_STATE, action);
  ...
  it('should create child state for array child', () => {
    const value = ['A'];
    const action = AddGroupControlAction({controlId: FORM_CONTROL_ID, name: 'inner5', value});
    const resultState = addControlReducer(INITIAL_STATE, action);
  ...
  it('should throw if trying to add existing control', () => {
    const action = AddGroupControlAction({controlId: FORM_CONTROL_ID, name: 'inner', value: ''});
  ...
  it('should forward actions to children', () => {
    ...
    const value = 'B';
    const action = AddGroupControlAction({controlId: state.controls.inner.id, name: 'inner2', value});
