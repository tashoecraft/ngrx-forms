...
describe(`form array ${addControlReducer.name}`, () => {
  ...
  it('should create child state at the given index', () => {
    const value = 'B';
    const action = AddArrayControlAction({controlId: FORM_CONTROL_ID, value, index: 1});
    const resultState = addControlReducer(INITIAL_STATE, action);
  ...
  it('should create child state at the start', () => {
    const value = 'B';
    const action = AddArrayControlAction({controlId: FORM_CONTROL_ID, value, index: 0});
    const resultState = addControlReducer(INITIAL_STATE, action);
    expect(resultState.value).toEqual([value, '', '']);
  ...
  it('should throw if trying to add control at out of bounds index', () => {
    const action = AddArrayControlAction({controlId: FORM_CONTROL_ID, value: '', index: 3});
  ...
  it('should update deeply nested child IDs after an insertion', () => {
    ...
    const action = AddArrayControlAction({controlId: FORM_CONTROL_ID, value, index: 0});
    const resultState = addControlReducer(INITIAL_DEEPLY_NESTED_GROUPS_FORM_STATE, action);
    ...
    resultState.controls.forEach((control: any, index: any) => {
      ...
      control.controls.deep.controls.inners.controls.forEach((deepControl: any, deepIndex: any) => {
