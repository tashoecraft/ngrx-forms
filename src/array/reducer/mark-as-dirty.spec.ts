...
describe(`form array ${markAsDirtyReducer.name}`, () => {
  ...
  it('should mark itself and all children recursively as dirty', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).toEqual(INITIAL_STATE_DIRTY);
  });

  it('should not update state if all children are marked as dirty recursively', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE_DIRTY, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should mark children as dirty if the group itself is already marked as dirty', () => {
    ...
    const resultState = markAsDirtyReducer(state, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).toEqual(INITIAL_STATE_DIRTY);
  });
  ...
  it('should mark group children as dirty', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE_NESTED_GROUP, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isDirty).toEqual(true);
    expect(resultState.controls[0].isPristine).toEqual(false);
  });

  it('should mark array children as dirty', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE_NESTED_ARRAY, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.controls[0].isDirty).toEqual(true);
    expect(resultState.controls[0].isPristine).toEqual(false);
  });

  it('should forward actions to children', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE, MarkAsDirtyAction({controlId: INITIAL_STATE.controls[0].id}));
