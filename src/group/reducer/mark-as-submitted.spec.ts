...
describe(`form group ${markAsSubmittedReducer.name}`, () => {
  ...
  it('should mark itself and all children recursively as submitted', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE_FULL, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should not update state if all children are marked as submitted recursively', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE_FULL_SUBMITTED, MarkAsSubmittedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should forward actions to children', () => {
    const resultState = markAsSubmittedReducer(INITIAL_STATE, MarkAsSubmittedAction({controlId: INITIAL_STATE.controls.inner.id}));
