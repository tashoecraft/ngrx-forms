...
describe(`form array ${setValueReducer.name}`, () => {
  ...
  it('should aggregate child values', () => {
    const value = 'A';
    const resultState = setValueReducer(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_0_ID, value}) as any);
    expect(resultState.value).toEqual([value, '']);
  });
  ...
  it('should aggregate child values for group children', () => {
    const value = { inner: 'A' };
    const resultState = setValueReducer(INITIAL_STATE_NESTED_GROUP, SetValueAction({controlId: FORM_CONTROL_0_ID, value}) as any);
  ...
  it('should aggregate child values for array children', () => {
    const value = ['A'];
    const resultState = setValueReducer(INITIAL_STATE_NESTED_ARRAY, SetValueAction({controlId: FORM_CONTROL_0_ID, value}) as any);
    expect(resultState.value).toEqual([value, ['']]);
  });
  ...
  it('should throw if trying to set date as value', () => {
    const action = SetValueAction({controlId: FORM_CONTROL_ID, value: new Date() as any});
