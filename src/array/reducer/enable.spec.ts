...
describe(`form array ${enableReducer.name}`, () => {
  ...
  it('should not update state if all children are enabled recursively', () => {
    const resultState = enableReducer(INITIAL_STATE, EnableAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should forward actions to children', () => {
    ...
    const resultState = enableReducer(state, EnableAction({controlId: state.controls[0].id}));
