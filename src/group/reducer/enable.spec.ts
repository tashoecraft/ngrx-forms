...
describe(`form group ${enableReducer.name}`, () => {
  it('should enable itself and all children recursively', () => {
    ...
    const resultState = enableReducer(state, EnableAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).toEqual(INITIAL_STATE_FULL);
  });

  it('should not update state if all children are enabled recursively', () => {
    const resultState = enableReducer(INITIAL_STATE_FULL, EnableAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should forward actions to children', () => {
    ...
    const resultState = enableReducer(state, EnableAction({controlId: state.controls.inner2.id}));
