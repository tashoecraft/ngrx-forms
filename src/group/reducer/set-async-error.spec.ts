...
describe(`form group ${setAsyncErrorReducer.name}`, () => {
  ...
  it('should update state with error', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_ID, name, value}));
  ...
  it('should remove pending validation without changing the error if error value is equal', () => {
    ...
    const state = { ...INITIAL_STATE, isValid: false, isInvalid: true, errors: { [`$${name}`]: value }, pendingValidations: [name], isValidationPending: true };
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_ID, name, value: { ...value }}));
    expect(resultState.errors[`$${name}`]).toBe(value);
    expect(resultState.pendingValidations).toEqual([]);
    expect(resultState.isValidationPending).toBe(false);
  });
  ...
  it('should aggregate child errors', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER_ID, name, value}));
  ...
  it('should aggregate child errors for group children', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER3_ID, name, value}));
  ...
  it('should aggregate child errors for array children', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER5_ID, name, value}));
  ...
  it('should aggregate multiple child errors', () => {
    ...
    let resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER_ID, name: name1, value: value1}));
    resultState = setAsyncErrorReducer(resultState, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER3_ID, name: name2, value: value2}));
  ...
  it('should track child errors and own errors when own errors are changed', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_ID, name: name1, value: value1}));
  ...
  it('should track own errors and child errors when child errors are changed', () => {
    ...
    const resultState = setAsyncErrorReducer(state, SetAsyncErrorAction({controlId: FORM_CONTROL_INNER_ID, name: name2, value: value2}));
    expect(resultState.errors).toEqual({ [`$${name1}`]: value1, _inner: { [`$${name2}`]: value2 } });
  });
