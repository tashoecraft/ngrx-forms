...
import {  FormControlState, FormGroupState } from './state';
...
describe(formStateReducer.name, () => {
  it('should apply the action to controls', () => {
    const resultState = formStateReducer<FormGroupValue['inner']>(INITIAL_STATE.controls.inner, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
    expect(resultState).not.toBe(INITIAL_STATE.controls.inner);
  });

  it('should apply the action to groups', () => {
    const resultState = formStateReducer<FormGroupValue>(INITIAL_STATE, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState).not.toBe(INITIAL_STATE);
  });

  it('should apply the action to arrays', () => {
    const resultState = formStateReducer<FormGroupValue['inner5']>(INITIAL_STATE.controls.inner5, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER5_ID}));
...
describe(createFormStateReducerWithUpdate.name, () => {
  ...
  it('should apply multiple provided function objects one after another', () => {
    ...
    )(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_INNER_ID, value: 'D'}));
...
describe(onNgrxForms.name, () => {
  it('should call the reducer for controls', () => {
    ...
    const resultState = onNgrxForms<typeof state>().reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
  ...
  it('should call the reducer for groups', () => {
    ...
    const resultState = onNgrxForms<typeof state>().reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should call the reducer for arrays', () => {
    ...
    const resultState = onNgrxForms<typeof state>().reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER5_ID}));
  ...
  it('should work with createReducer', () => {
    ...
    let resultState = reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
    expect(resultState.control).not.toBe(INITIAL_STATE.controls.inner);

    resultState = reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.group).not.toBe(INITIAL_STATE);

    resultState = reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER5_ID}));
  ...
  it('should work with createReducer for top-level form state', () => {
    ...
    const resultState = reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.id).toBe(INITIAL_STATE.id);
    expect(resultState).not.toBe(INITIAL_STATE);
  });
});
...
describe(onNgrxFormsAction.name, () => {
  ...
  const reducer = createReducer(
    state,
    on(MarkAsTouchedAction, s => ({ ...s })),
  );

  it('should call the reducer for the correct action type', () => {
    const resultState = reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
    expect(resultState).not.toBe(state);
  });

  it('should not call the reducer for the wrong action type', () => {
    const resultState = reducer(state, MarkAsDirtyAction({controlId: FORM_CONTROL_INNER_ID}));
  ...
  it('should provide the action of the right type to the reducer', () => {
    const reducer = createReducer(
      state,
      on(MarkAsTouchedAction, (state: any, action: any) => {
        expect(action instanceof MarkAsTouchedAction).toBe(true);
        expect(action.controlId).toBe({controlId: FORM_CONTROL_INNER_ID});
        return state;
      }),
    );

    reducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
  });
  ...
  it('should call the reducer in conjuction with onNgrxForms', () => {
    const reducer = createReducer(
      state,
      onNgrxForms(),
      on(MarkAsTouchedAction, (state, action) => {
        expect(action instanceof MarkAsTouchedAction).toBe(true);
...
describe(wrapReducerWithFormStateUpdate.name, () => {
  ...
  xit('should update a non-nested array after the reducer', () => {
  ...
  it('should work with createReducer', () => {
    ...
    let resultState = wrappedReducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER_ID}));
    expect(resultState.control).not.toBe(INITIAL_STATE.controls.inner);

    resultState = wrappedReducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_ID}));
    expect(resultState.group).not.toBe(INITIAL_STATE);

    resultState = wrappedReducer(state, MarkAsTouchedAction({controlId: FORM_CONTROL_INNER5_ID}));
    expect(resultState.array).not.toBe(INITIAL_STATE.controls.inner5);
  });
});
