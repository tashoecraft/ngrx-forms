...
describe(`form group ${markAsDirtyReducer.name}`, () => {
  ...
  it('should mark itself and all children recursively as dirty', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE_FULL, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should not update state if all children are marked as dirty recursively', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE_FULL_DIRTY, MarkAsDirtyAction({controlId: FORM_CONTROL_ID}));
  ...
  it('should forward actions to children', () => {
    const resultState = markAsDirtyReducer(INITIAL_STATE, MarkAsDirtyAction({controlId: INITIAL_STATE.controls.inner.id}));
