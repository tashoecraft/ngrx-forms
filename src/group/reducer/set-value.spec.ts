...
describe(`form group ${setValueReducer.name}`, () => {
  ...
  it('should remove child states on demand', () => {
    const value = { inner: 'A', inner2: 'B' };
    let resultState = setValueReducer(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_ID, value}));
    const value2 = { inner: 'A' };
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_ID, value: value2}));
  ...
  it('should remove child states on demand when value is empty', () => {
    ...
    const resultState = setValueReducer(state, SetValueAction({controlId: id, value: {}}));
  ...
  it('should aggregate child values', () => {
    const value = 'A';
    const resultState = setValueReducer(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_INNER_ID, value}) as any);
  ...
  it('should aggregate child values for group children', () => {
    let resultState = setValueReducer(
      INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_ID, value: { inner: 'A', inner3: { inner4: 'C' } }}),
    );
    const value = { inner4: 'D' };
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER3_ID, value}));
    expect(resultState.value.inner3).toEqual(value);
  });

  it('should aggregate child values for array children', () => {
    let resultState = setValueReducer(
      INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_ID, value: { inner: 'A', inner5: ['C'] }}),
    );
    const value = ['D'];
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER5_ID, value}) as any);
  ...
  it('should not mark state as dirty if array child value is updated', () => {
    ...
    const value = ['D'];
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER5_ID, value}));
    expect(resultState.isDirty).toEqual(false);
  ...
  it('should aggregate nested child values in groups', () => {
    ...
    const value = 'D';
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER4_ID, value}) as any);
  ...
  it('should aggregate nested child values in arrays', () => {
    ...
    const value = 'D';
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER5_0_ID, value}));
  ...
  it('should not mark state as dirty if nested child value in group is updated', () => {
    ...
    const value = 'D';
    resultState = setValueReducer(resultState, SetValueAction({controlId: FORM_CONTROL_INNER4_ID, value}));
    expect(resultState.isDirty).toEqual(false);
