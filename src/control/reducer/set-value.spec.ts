...
describe('form control setValueReducer', () => {
  ...
  it('should throw for date values', () => {
    ...
    expect(() => setValueReducer(state, SetValueAction({controlId: FORM_CONTROL_ID, value}))).toThrowError();
  });

  it('should throw if value is not supported', () => {
    const value = {};
    expect(() => setValueReducer(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_ID, value}))).toThrowError();
  });
  ...
  it('should throw if boxed value is not serializable', () => {
    ...
    expect(() => setValueReducer(INITIAL_STATE, SetValueAction({controlId: FORM_CONTROL_ID, value} as any))).toThrowError();
  });
});
